...
export default function Sequence(source : IterableObject) {
  ...
  this.stream = (downstream : DownstreamPipeline) : NullableBoolean => {
    const iterator : IteratorLike = getIterator(this.source);
...
Sequence.prototype.head = function take(count? : number) : Sequence {
 ...
 */
Sequence.prototype.reduce = function reduce<T : IterableObject>(aggregator : Aggregator<T>,
                                                                initial : T) : T {
