...
import dropWhilePipeline from './api/drop-while';
import firstPipeline from './api/first';
import lastPipeline from './api/last';
import mapPipeline from './api/map';
import takePipeline from './api/take';
import takeWhilePipeline from './api/take-while';
...
Sequence.prototype.skip = function drop(count : number = 1) : Sequence {
 ...
 * @alias skipWhile
 *
 * Drops items based on the provided `condition`.
 * @param  {DropCondition} condition the condition to determine whether the item should be dropped.
 * @return {Sequence}                a sequence that can be used to chain further manipulations.
 *
 * @example
 * Sequence.from([1, 2, 3, 4]).dropWhile((value) => value < 2).toArray()    // [3, 4]
 * Sequence.from([1, 2, 3, 4]).dropWhile((_, index) => index < 1).toArray() // [2, 3, 4]
 * Sequence.from([1, 2, 3, 4]).dropWhile((_, index) => index > 2).toArray() // [1, 2]
 */
Sequence.prototype.dropWhile =
Sequence.prototype.skipWhile = function dropWhile(condition : DropCondition) : Sequence {
  return dropWhilePipeline(this, condition);
};
 ...
 * @return {any} the first element of the sequence.
 *
 * @example
 * Sequence.from([1, 2, 3, 4]).first(); // 1
 */
Sequence.prototype.first =
Sequence.prototype.head = function first() : any {
  return firstPipeline(this);
};
 ...
 * @return {any} the last element of the sequence.
 *
 * @example
 * Sequence.from([1, 2, 3, 4]).last(); // 4
 */
Sequence.prototype.last =
Sequence.prototype.tail = function last() : any {
  return lastPipeline(this);
};
...
Sequence.prototype.yield = function take(count : number = 1) : Sequence {
 ...
 * @alias yieldWhile
 *
 * Takes items based on the provided `condition`.
 * @param  {TakeCondition}  condition the condition to determine whether the item should be taken.
 * @return {Sequence}                 a sequence that can be used to chain further manipulations.
 *
 * @example
 * Sequence.from([1, 2, 3, 4]).takeWhile((value) => value < 3).toArray();    // [1, 2]
 * Sequence.from([1, 2, 3, 4]).takeWhile((_, index) => index < 1).toArray(); // [1]
 * Sequence.from([1, 2, 3, 4]).takeWhile((_, index) => index > 2).toArray(); // [3, 4]
 */
Sequence.prototype.takeWhile =
Sequence.prototype.yieldWhile = function takeWhile(condition : TakeCondition) : Sequence {
  return takeWhilePipeline(this, condition);
};
